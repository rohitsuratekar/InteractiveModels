...
def open2_with_pip2_reverse(concentrations: list, time: tuple, *args) -> list:
    assert len(concentrations) == 8, "You should provide all concentrations"
    enzyme_list = args[0]  # type:dict
    feed_para = args[1]
    if feed_para is not None:
    ...
    d_pmpi = pitp.react_with(erpi, feed_para) - pi4k.react_with(pmpi,
                                                                feed_para)
    d_pi4p = pi4k.react_with(pmpi, feed_para) \
             - pip5k.react_with(pi4p, feed_para) + p5tase.react_with(pip2,
                                                                     feed_para)
    d_pip2 = pip5k.react_with(pi4p, feed_para) \
             - plc.react_with(pip2, feed_para) - p5tase.react_with(pip2,
                                                                   feed_para)
    d_dag = plc.react_with(pip2, feed_para) - dagk.react_with(dag, feed_para) \
            + laza.react_with(pmpa, feed_para) - sink.react_with(dag,
                                                                 feed_para)
